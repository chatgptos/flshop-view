...
<script>
	/**
	 * WanlShare 分享 
	 * @description 分享组件 上海蜂雷网络科技有限公司 https://www.flbooth.com（除万联官方内嵌系统，未经授权严禁使用）
	...
	export default {
		...
		methods: {
			async share(e) {
				if (e.id == 'poster') {
					this.$flbooth.to('/pages/user/qrcode/qrcode?url='+encodeURIComponent(this.href));
				} else if (e.id == 'link') {
					this.$flbooth.copy(this.href);
				} else if (e.id == 'report') {
					this.complaint();
				} else {
					...
					switch (this.shareType){
						...
						case 5:
							...
							shareOPtions.imageUrl = this.image ? this.image : $flbooth.appstc('/qrcode/logo.png');
							shareOPtions.title = this.shareTitle;
