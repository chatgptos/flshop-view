						...
						<view class="swiper-item">
							<scroll-view class="scroll-view-item" scroll-y="true">
								<view class="address_item" v-for="(item,index) in cityAreaArray[swiper_index]" :key="index" @tap="address_item_click(swiper_index,index)">
									<image v-if="selectIndexArr[swiper_index] === index" class="address_item_icon" :src="$flbooth.appstc('/common/img_gou3x.png')" mode=""></image>
									{{item.name}}
								</view>
							</scroll-view>
						</view>
...
<script>
	export default {
		...
		created() {
			uni.request({
			    url: `${this.$flbooth.config('appurl')}/flbooth/common/area`,
			    success: (res) => {
					if(res.data.data.length > 0){
						this.cityData = res.data.data
						this.cityAreaArray.push(res.data.data)
					}else{
						this.$flbooth.msg('数据表area为空，请在后台安装示例服务')
					}
			    }
			});
		},
