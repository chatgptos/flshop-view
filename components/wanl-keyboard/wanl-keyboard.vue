<template>
	<view id="number-keyboard-component" class="numberkeyboard" v-show="numberKeyboardPopupVisible">
		<view class="title" @tap="close"><image :src="$flbooth.appstc('/common/img_down3x.png')" style="width: 30upx;height: 30upx;"></image></view>
		<view class="keys">
			<view class="key button" v-for="num in config.loop" :key="num.key" @tap="number(num.number)">{{ num.number }}</view>
			<view class="key button" @tap="empty" style="background: #f5f5f5;"><image :src="$flbooth.appstc('/common/img_empty3x.png')" style="width: 50upx;height: 50upx;"></image></view>
			<view class="key button" @tap="number(0)">0</view>
			<view class="key button" @tap="del" style="background: #f5f5f5;"><image :src="$flbooth.appstc('/common/img_delete3x.png')" style="width: 50upx;height: 50upx;"></image></view>
		</view>
