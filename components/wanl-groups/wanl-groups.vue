	...
	<scroll-view
		...
		<view v-if="childlist.length > 0" class="wanlgroups-container__menu bg-white margin-bj padding-bj radius-bock">
			<block v-if="itemId === 0">
				...
				>
					<image :src="$flbooth.oss(item.image, 50, 50)"></image>
	...
	export default{
		...
		methods:{
			async loadData(type) {
				...
				// 请求数据
				await this.$api.get({
					url: '/flbooth/groups/product/recommend',
					data: {
						cid: this.itemId,
			...
			productList(category_id, category_name) {
				this.$flbooth.to(`/pages/product/list?type=groups&data=${JSON.stringify({ category_id: category_id, category_name: category_name })}`);
			},
			handleGroups(id) {
