<template>
	<scroll-view class="flbooth-find-container" :style="{ height: `${windowHeight}px` }" scroll-y enable-back-to-top
		...
		<view v-if="dataList.length > 0" class="flbooth-find-container__list">
			<view class="item margin-bj" v-for="(item, index) in dataList" :key="item.id">
				<view class="time">
					<text v-if="timeToFind(item.created)"
						class="day text-xxl text-bold margin-right-xs">{{timeToFind(item.created)}}</text>
					<block v-else>
						<text class="day text-xxl text-bold margin-right-xs">{{item.created_date}}</text>
						<text
							class="month wanl-gray-light">{{parseInt($flbooth.timeFormat(item.created, 'mm'))}}月</text>
					</block>
				</view>
						...
						<block v-for="(img, kes) in item.images" :key="kes">
							<block v-if="item.type === 'live'">
								<view class="list" :class="[kes == 0 ? 'item-live':'']"
									:style="{ backgroundImage: 'url(' + $flbooth.oss(img, 200, 0) + ')' }">
									<block v-if="kes == 0">
										<view class="play">
											<text
							...
							<block v-else>
								...
								<view 
									v-else 
									class="list" 
									:style="{ backgroundImage: `url(${$flbooth.oss(img, 200, 200)})` }"
									@click.stop="previewImage(item.images, kes)" 
								/>
							</block>
						</block>
