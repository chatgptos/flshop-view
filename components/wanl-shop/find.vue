<template>
	...
	>
			...
			>
				<view class="text-center">
					...
					<view class="margin-top-sm">
						<button class="cu-btn bg-orange" @tap="$flbooth.auth('/pages/find')">
							马上登陆
						</button>
					</view>
				</view>
		...
		<block v-else>
			<view
				class="flbooth-find-container__list  wanl-find margin-top-sm"
			...
			>
				<view class="margin-lr-bj bg-white radius-bock padding-lr-bj padding-top-bj">
							...
							<view class="avatar" @tap="handleUser(find.user_no)">
								<view
									...
									:style="{
										backgroundImage:
											'url(' +
											$flbooth.oss(find.shop.avatar, 35, 35, 2, 'avatar') +
											')'
									}"
								></view>
								<view class="content">
									...
									<view class="wanl-gray text-min">
										{{ $flbooth.timeToDate(find.created) }}
									</view>
								</view>
							</view>
							<view class="menu">
							...
							<view class="avatar" @tap="handleUser(find.user_no)">
								<view
									...
									:style="{
										backgroundImage:
											'url(' +
											$flbooth.oss(find.user.avatar, 35, 35, 2, 'avatar') +
											')'
									}"
								></view>
								<view class="content">
						...
						<block v-for="(img, kes) in find.images" :key="kes">
							<block v-if="find.type === 'live'">
								<view
									...
									:style="{
										backgroundImage: 'url(' + $flbooth.oss(img, 200, 0) + ')'
									}"
								>
									<block v-if="kes == 0">
										<view class="play">
										...
										<!-- <view class="number text-white">
														<view>{{$flbooth.toFormat(find.live.goods_ids.split(',').length,'hundred')}}</view>
														<text>宝贝</text> 
													</view> -->
										<view
							...
							<block v-else-if="find.type === 'video'">
								<view class="item item-1 text-sl radius-bock" :style="{ backgroundImage: 'url(' + $flbooth.oss(img, 200, 0) + ')' }" @tap="onFind(find)">
					...
					<scroll-view class="scroll-view margin-top-sm" scroll-x="true">
						...
						>
							<view class="img">
								<image lazy-load :src="$flbooth.oss(goods.image, 50, 50)"></image>
							</view>
...
export default {
	...
	methods: {
		...
		handleUser(user_no) {
			this.$flbooth.to(`/pages/apps/find/user?id=${user_no}`);
		},
		//预览图片
