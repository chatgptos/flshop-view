<template>
	<scroll-view
		class="flbooth-category-container"
		:style="{
		...
		<!-- 菜单 -->
		<view v-if="childlist.length > 0" class="flbooth-category-container__menu bg-white margin-bj padding-bj radius-bock">
			<view v-for="(item, index) in childlist" :key="item.id" class="item" @tap="productList(item.id, item.name)">
				<image lazy-load :src="$flbooth.oss(item.image, 50, 50)"></image>
		...
		<!-- 秒杀 -->
		<view v-if="seckillList.length > 0" class="flbooth-category-container__seckill bg-white margin-bj padding-bj radius-bock">
			
		</view>
		<!-- 拼团 -->
		<view v-if="groupsList.length > 0" 
			class="flbooth-category-container__groups bg-white margin-bj padding-bj radius-bock"
			:style="{backgroundImage: `url(${$flbooth.appstc('/show/grueps-top-bg3x.png')})`}"
		>
			...
			<scroll-view class="list" scroll-x>
                <view class="item text-center margin-right-sm" v-for="(item, index) in groupsList" :key="item.id" @tap="handleGroupsGoods(item.id)">
					<image lazy-load class="radius-bock" :src="$flbooth.oss(item.image, 50, 50)"></image>
					<view class="text-cut">
						{{item.title}}
					</view>
		...
		<!-- 分隔符 -->
		<view class="flbooth-category-container__separate">
	...
	export default{
		...
		methods:{
			...
			productList(category_id, category_name) {
				this.$flbooth.to(`/pages/product/list?data=${JSON.stringify({ category_id: category_id, category_name: category_name })}`);
			},
			handleGroups(id) {
				this.$flbooth.to(`/pages/apps/groups/index?id=${id}`);
			},
