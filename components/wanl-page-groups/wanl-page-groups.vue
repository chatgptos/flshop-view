<template>
	<view class="wanlpage-groups" :style="[pageData.style]">
		<view
			...
			:style="{ backgroundImage: `url(${$flbooth.oss( pageData.params.groupsBackground, 0, 50, 1, 'transparent', 'png' )})` }"
		>
			<view class="head">
				<view class="title">
					<text
			...
			<view class="list">
				<view class="item" v-for="(item, index) in data" :key="item.id" @tap="handleGoods(item.id)">
					<image :src="$flbooth.oss(item.image, 100, 100)" mode="aspectFill"></image>
