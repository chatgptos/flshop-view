<template>
	<view class="wanlpage-classify" :style="[pageData.style]">
		<view class="bg" :style="{backgroundImage: `url(${$flbooth.oss(pageData.params.classifyBackground, 0, 50, 1, 'transparent', 'png')})`}">
				...
				<view class="cu-item" v-for="(item,index) in data" :key="index">
					...
					<view class="goods margin-top-bj">
						<block v-for="(goods, keys) in item.list" :key="keys">
							<image class="radius" :src="$flbooth.oss(goods.image, 70, 65)" @tap="onGoods(goods.id)" mode="aspectFill"/>
						</block>
					</view>
				</view>
			</view>
