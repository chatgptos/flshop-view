<template>
	<view class="wanlpage-activity" :style="[pageData.style]">
		<view class="bg" :style="{backgroundImage: `url(${$flbooth.oss(pageData.params.activityBackground, 0, 50, 1, 'transparent', 'png')})`}">
				...
				<view class="cu-item" v-for="(item, index) in data" :key="index">
					...
					<view class="goods margin-top-bj">
						<block v-for="(groups, keys) in item.list" :key="keys">
							<image class="radius" :src="$flbooth.oss(groups.image, 70, 65)" @tap="handleItem(item.activity, groups.id)" mode="aspectFill"/>
						</block>
					</view>
				</view>
			</view>
