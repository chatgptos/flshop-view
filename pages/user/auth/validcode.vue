	...
	methods: {
		...
		login() {
			...
			if (this.event == 'resetpwd') {
				this.$flbooth.to(`resetpwd?mobile=${this.mobile}&captcha=${this.code.join("")}&url=${this.pageroute}`);
				uni.hideLoading();
			}
			// 绑定手机号
