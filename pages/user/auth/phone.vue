...
export default {
	...
	methods: {
		formSubmit: function(e) {
			...
			if (checkRes) {
				this.$flbooth.to(`validcode?event=mobilelogin&mobile=${e.detail.value.mobile}&url=${this.pageroute}`,'slide-in-bottom',200);
			} else {
		...
		name() {
			this.$flbooth.to(`name?name=${this.mobile}&url=${this.pageroute}`);
		},
		register() {
