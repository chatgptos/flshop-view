				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view class="wanl-coupon">
						<view v-for="(coupon, seat) in tag.cardList" :key="coupon.id" :class="[coupon.state == 1 ? coupon.type : 'wanl-gray']" class="item margin-bj radius-bock" @tap="onDetails(coupon)">
							...
							<image v-if="coupon.state == 2" :src="$flbooth.appstc('/coupon/img_coupon_beused_3x.png')" class="coupon-sign"></image>
							<image v-if="coupon.state == 3" :src="$flbooth.appstc('/coupon/img_coupon_failure_3x.png')" class="coupon-sign"></image>
								...
								<view class="content text-min">
									<view class="wanl-gray">
										...
										<view v-else-if="coupon.pretype == 'appoint' && coupon.validity != 0">
											<text class="wlIcon-dot"></text>{{$flbooth.timeFormat(coupon.created + coupon.validity * 86400)}} 前有效
										</view>
										<view v-else>
...
export default {
	...
	methods: {
		...
		onApply(id){
			this.$flbooth.to(`/pages/user/coupon/apply?id=${id}&state=true`);
		},
		onDetails(data){
