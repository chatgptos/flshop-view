<template>
	<view>
		...
		<view class="wanlian">
			<image :src="$flbooth.oss(iniData.logo)" mode="aspectFill"></image>
		...
		<view class="cu-list menu margin-xl">
			<view class="cu-item arrow" @tap="$flbooth.to('/pages/user/feedback/feedback')">
				<view class="content">
					<text>意见与反馈</text>
				</view>
			</view>
	...
	methods: {
		...
		update(){
			if (this.$store.state.update.update) {
				...
				this.$flbooth.msg(`当前 ${this.$store.state.common.version} 最新，暂无新版本更新！`);
			}
		}
	}
};
