		...
		<view class="wanl-order"> 
			...
			<view class="header">
				...
				<view class="content">
					...
					<image :src="$flbooth.appstc('/order/'+getImg(orderData.state))" class="status-img" mode="widthFix"></image>
				</view>
			</view>
			<!-- 详情 -->
			...
			<view class="lists bg-white"  v-if="orderData.order_no">
				...
				<view class="cu-list menu-avatar">
					<block v-for="(item, index) in orderData.goods" :key="item.id">
						<view class="cu-item">
							<view class="cu-avatar radius" :style="{ backgroundImage: 'url(' + $flbooth.oss(item.image, 77, 77) + ')' }" @tap="onGoods(item.goods_id)"></view>
			...
			<view class="order bg-white margin-top-bj padding-bj" v-if="orderData.order_no">
				...
				<view class="text-sm">
					...
					<view class="item">
						...
						<text> {{orderData.created_text}} </text>
					</view>
					<block v-if="infoMore">
...
export default {
	...
	methods: {
		...
		async loadData(data) {
			this.$api.get({
				url: '/flbooth/order/getOrderInfo',
				data: data,
				success: res => {
					this.orderData = res;
					...
					if(res.state == 0){
						...
						var diff = (res.created + res.config.cancel * 86400) - Date.parse(new Date()) / 1000;
		...
		onRefund(order_id, refund_status, refund_id, index){
			if (refund_status == 0) { //申请退款
				this.$flbooth.to(`/pages/user/refund/apply?data=${JSON.stringify({
				...
				this.$flbooth.to(`/pages/user/refund/details?id=${refund_id}`);
			}
		},
		// 折叠项
		toMore(){
