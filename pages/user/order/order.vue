				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<view v-for="(order, okey) in tag.orderList" :key="order.id" class="order-item radius-bock">
						...
						<view class="goods-box" v-for="(goods, gkey) in order.goods" :key="goods.id" @tap="orderDetails(order.id)">
							<view class="cu-avatar xl margin-right-bj radius" :style="{backgroundImage: 'url('+$flbooth.oss(goods.image, 70, 70)+')'}"> </view>
...
export default {
	...
	methods: {
		...
		loadData(source) {
			...
			// 获取列表
			this.$api.get({
				url: '/flbooth/order/getOrderList',
				data: {
					state: state,
		...
		commentOrder(id){
			this.$flbooth.to('/pages/user/order/comment?order_id=' + id);
		},
		//支付订单
		paymentOrder(id){
			this.$flbooth.to(`/pages/user/money/pay?order_id=${id}&order_type=goods`);
		},
		//确认收货- 全局
		confirmOrder(item){
			uni.showLoading({
			...
			// 获取列表
			this.$api.post({
				url: '/flbooth/order/confirmOrder',
				data: {
					id: item.id
				},
				success: res => {
		...
		async refreshList(address_id, order_id) {
			...
			// 获取列表
			this.$api.post({
				url: '/flbooth/order/editOrderAddress',
				data: {
					id: order_id,
