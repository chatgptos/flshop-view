...
export default {
	...
	methods: {
		async loadData(reload, refresh) {
			this.$api.get({
				url: '/flbooth/address/getaddress',
		...
		async delAddress(id, key) {
			...
			this.$api.post({
				url: '/flbooth/address/deladdress',
				data: { id: id }
			});
		},
		//添加或修改成功之后回调
		...
		checkAddress(item) {
			if (this.source == 1) {
				this.$flbooth.prePage().loadData(item.id);
				this.$flbooth.back(1);
			}else if(this.source == 2){ //我的订单修改地址  地址ID   返回订单ID
				this.$flbooth.prePage().refreshList(item.id, this.order_id);
				this.$flbooth.back(1);
			}
		},
		// 新增地址
		addAddress(type, item) {
			this.$flbooth.to(`/pages/user/address/addressManage?type=${type}&data=${JSON.stringify(item)}`);
		},
		// ListTouch触摸开始
		ListTouchStart(e) {
