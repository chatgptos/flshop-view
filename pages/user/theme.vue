<template>
	<view class="wanlgroups-user-container">
		...
		<view class="wanlgroups-user-container__main"
			:style="{backgroundColor: themeData.color, backgroundImage: `url(${$flbooth.oss(themeData.image, 0, 50, 1, 'transparent', 'png')})`}">
			<view class="head text-white" :style="{paddingTop: headHeight + 'px'}">
				<view class="userinfo">
					<view class="cu-avatar margin-right-bj round"
					:style="{backgroundImage: `url(${$flbooth.oss(user.avatar, 62, 62, 2, 'avatar')})`}"></view>
					<view>
						<view class="text-xl margin-bottom-xs">{{user.nickname}}</view>
						<view class="text-min text-white-agba">
							<text>账户号：</text>
							<text>{{themeData.user_no}}</text>
							<text class="wlIcon-fuzhi margin-left-sm" @click.stop="$flbooth.copy(themeData.user_no)"></text>
						</view>
					</view>
				</view>
			</view>
		...
		<scroll-view class="wanlgroups-user-container__list bg-white padding-bj" scroll-y
			...
			<view class="content">
				...
				>
					<image :src="$flbooth.oss(item.image, 0, 50)" mode=""></image>
...
<script>
	...
	export default {
		...
		methods: {
			async loadData() {
				this.$api.post({
					url: '/flbooth/find/user/getTheme',
					success: res => {
						this.themeData = res;
					}
				});
			...
			changeTheme(){
				this.$flbooth.prePage().editTheme({
