					...
					<view v-for="(item, footprint) in data.arrData" :key="footprint">
						...
						<view class="box">
							<view class="item" v-for="(goods, index) in item" :key="index">
								<image :src="$flbooth.oss(goods.goods.image, 136, 126)" @tap="handleGoods(goods.goods_id)"></image>
	...
	export default {
		...
		methods: {
			...
			formatData(data) {
				var groups = {};
				data.forEach(res => {
					var cate = this.getLocalTime(res.created);
					if (!groups[cate]) {
						groups[cate] = [];
					}
