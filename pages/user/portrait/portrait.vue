<template>
	<view class="content">
		...
		<view class="text-center padding-top-xl" :style="{backgroundImage: `url(${$flbooth.maks()})`}">
			<view @tap="chooseImage" class="cu-avatar avatar" v-if="path" :style="{ backgroundImage: 'url(' + path + ')' }"></view>
			<view @tap="chooseImage" class="cu-avatar avatar" v-else :style="{ backgroundImage: 'url(' + $flbooth.oss(user.avatar, 100, 100) + ')' }"></view>
