	...
	<view class="wanl-signin">
		...
		<view class="header wanl-bg-redorange">
			<view class="user margin-left">
				<view class="cu-avatar lg round" :style="{ backgroundImage: 'url(' + $flbooth.oss(user.avatar, 52, 52, 2, 'avatar') + ')' }"></view>
		...
		<wanl-modal custom :show="attendanceModalVisible">
			<view class="attendance-modal">
				<view class="modal-header">
					<image :src="$flbooth.appstc('/common/cup.png')" mode="widthFix"></image>
				</view>
			...
			this.$api.get({
				...
				success: res => {
					...
					res.list.forEach(item => {
						this.selected.push({ date: this.timestampToTime(item.created), info: '已签' });
					});
