	...
	<view class="wanl-order-list">
		...
		<view class="order-item radius-bock" v-for="(item, index) in dataList" :key="item.id">
			...
			<view class="goods-box" @tap="onRefund(item.id)">
				<view class="cu-avatar xl margin-right-bj radius" :style="{backgroundImage: 'url('+$flbooth.oss(item.goods.image, 70, 70)+')'}"> </view>
...
export default {
	...
	methods: {
		...
		onRefund(id){
			this.$flbooth.to(`/pages/user/refund/details?id=${id}`);
		}
	}
};
</script>
