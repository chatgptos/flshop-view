<template>
	<view>
		...
		<view class="bg-white padding-bj flex">
			<view class="cu-avatar xl margin-right-bj" :style="{backgroundImage: 'url('+$flbooth.oss(refund.goods.image, 70, 70)+')'}"> </view>
	...
	methods: {
		...
		async addData(){
			if (this.refund.price <= 0) {
				this.$flbooth.msg('价格不能为空');
				return false;
			}
			if (this.refund.price > this.amount.total) {
				this.$flbooth.msg('退款不能超过￥'+this.amount.total+'元');
				return false;
			}
			// 提交
			this.$api.post({
				url: '/flbooth/refund/editRefund',
				data: {
					id: this.refund.id,
