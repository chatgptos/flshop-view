<template>
	<view>
		...
		<view class="wanl-refund"> 
			<!-- 头部 -->
			<view class="header">
				<image :src="$flbooth.appstc('/order/img_detail_bg.png')" class="img-bg"></image>
				<view class="content">
					<view>
			...
			<view class="padding-bj flex">
				<view class="cu-avatar xl margin-right-bj" :style="{backgroundImage: 'url('+$flbooth.oss(refundData.goods.image, 70, 70)+')'}"> </view>
			...
			<view class="bg-white padding-bj text-sm">
				...
				<view class="item flex margin-top-bj">
					<text class="wanl-gray"> 退款时间： </text> <text> {{refundData.created_text}} </text>
				</view>
			</view>
...
export default {
	...
	methods: {
		async loadData(data) {
			this.$api.get({
				url: '/flbooth/refund/getRefundInfo',
				data: data,
				success: res => {
					this.refundData = res;
					...
					// 时间差的毫秒数 
					if(res.state == 0){
						var diff = (res.created + res.config.autoagree * 86400) - Date.parse(new Date()) / 1000;
		...
		// 退款历史
		refundLog(id){
			this.$flbooth.to(`/pages/user/refund/log?id=${id}`);
		},
		// 平台介入
		async arbitrationRefund(id){
			this.$api.get({
				url: '/flbooth/refund/arbitrationRefund',
				data: {id:id},
				success: res => {
					this.loadData({id:id});
				}
		...
		async closeRefund(id){
			this.$api.get({
				url: '/flbooth/refund/closeRefund',
				data: {id:id},
				success: res => {
		...
		async toExpress(){
			if (!this.returnData.express_no) {
				this.$flbooth.msg('运单号不能为空');
				return false;
			}
			if (!this.returnData.express_name) {
				this.$flbooth.msg('请选择快递公司');
				return false;
			}
			this.$api.post({
				url: '/flbooth/refund/toExpress',
		...
		// 修改退款
		editRefund(id){
			this.$flbooth.to(`/pages/user/refund/edit?id=${id}`);
		}
