<template>
	<view>
		...
		<view class="bg-white padding-bj flex">
			<view class="cu-avatar xl margin-right-bj" :style="{backgroundImage: 'url('+$flbooth.oss(goods.image, 70, 70)+')'}"> </view>
			...
			<view class="bg-white margin-top-bj">
				...
				<view class="grid col-4 grid-square flex-sub padding-lr">
					<view class="bg-img" v-for="(item, index) in refund.images" :key="index" @tap="viewImage(index)">
						<image :src="$flbooth.oss(item, 90, 90)" mode="aspectFill"></image>
...
export default {
	...
	onLoad(option) {
		...
		if(this.goods_number == 1 || this.freight_type == 2){
			let price = this.$flbooth.bcadd(data.goods.actual_payment, data.goods.freight_price);
			let info = `最多￥${price}`;
			if(parseInt(data.discount_price) > 0){
				info += `，其中总额￥${this.$flbooth.bcmul(data.goods.price, data.goods.number)}，优惠￥${data.goods.discount_price}`;
			}
	...
	methods: {
		async addData(){
			if (this.refund.money > this.amount.total) {
				this.$flbooth.msg(this.amount.info);
				return false;
			}
			if (this.refund.expressType == -1) {
				this.$flbooth.msg('请选择物流状态');
				return false;
			}
			if (this.refund.type == -1) {
				this.$flbooth.msg('请选择退款类型');
				return false;
			}
			if (this.refund.reason == -1) {
				this.$flbooth.msg('请选择退款原因');
				return false;
			}
			...
			this.$api.post({
				...
				success: res => {
					...
					// 跳转到退款详情页
					this.$flbooth.to(`/pages/user/refund/details?id=${res}`);
				}
			});
		},
		chooseImage(index) {
