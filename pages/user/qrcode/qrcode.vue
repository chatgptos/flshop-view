		...
		<view class="page-foot">
			<view class="template-list">
				<view class="template-item" v-for="(x, xi) in posterTemplates" :key="xi" @tap="create(x)" :class="{ checked: x.checked }" :style="{ width: x.thumbnail_width+'px' }">
					<image :src="$flbooth.oss(x.thumbnail_url, 0, 88)" />
				</view>
				<!-- #ifdef H5 -->
	...
	methods: {
				...
				success: res => {
					...
					res.forEach((item, index, arr) => {
						if (item.template != 'flboothqr') {
							// 远程下载背景
							uni.downloadFile({
								url: this.$flbooth.oss(item.background_url, 350, 0),
								success: e => {
							...
							// 远程下载图标
							uni.downloadFile({
								url: this.$flbooth.oss(item.logo_src, 40, 40),
								success: e => {
		...
		save() {
			uni.saveImageToPhotosAlbum({
				...
				success: () => {
					this.$flbooth.msg('保存成功');
				}
			});
		},
		flboothqrlist001(options) {
