	...
	<view class="notice">
		...
		<view class="cu-custom" :style="{height: $flbooth.wanlsys().height + 'px' }">
			<view class="cu-bar fixed bg-bgcolor" :style="{
				...
				<view class="action">
					<text class="wlIcon-fanhui1" style="margin-left: 0;" @tap="$flbooth.back(1)"></text>
					<!-- #ifdef MP -->
					<text class="wlIcon-shezhi" @tap="$flbooth.to('/pages/user/setting/notice')"></text>
					<!-- #endif -->
				</view>
		...
		<view class="wanl-msg" v-if="chat.list.length != 0">
			<view class="cu-list menu-avatar">
				<view class="cu-item" v-for="(item, index) in chat.list" :key="index" @tap="toChat(item.id)" :class="modalName=='move-box-'+ item.id?'move-cur':''" @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-' + item.id">
					<view class="cu-avatar round lg" :style="{ backgroundImage: 'url(' + $flbooth.oss(item.avatar, 100, 100) + ')' }"></view>
					<view class="content">
					...
					<view class="action">
						<view class="text-gray text-sm">{{$flbooth.timeToChat(item.created)}}</view>
