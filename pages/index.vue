<template>
	<view class="flbooth-container">
		<view
			v-if="common.modulesData.homeModules.page"
			class="flbooth-container__head"
			:style="{
				...
				backgroundImage: 'url(' + $flbooth.oss( common.modulesData.homeModules.page ? common.modulesData.homeModules.page.style.navigationBackgroundImage : '', 0, 50, 1, 'transparent', 'png' ) + ')'
			}"
		>
			<view :style="{ height: headHeight + 'px', paddingTop: headTop + 'px' }" >
				<view class="navigater flex align-center justify-between">
					...
					<view class="flex">
						<view class="margin-right-bj text-xxl position-relative" @tap="$flbooth.to('/pages/user/coupon/list')">
							<text class="wlIcon-coupon"/>
							<view class="cu-tag badge">减</view>
						</view>
						<view class="text-xxl" @tap="$flbooth.to('/pages/notice/notice')">
		...
		<view class="WANL-MODAL" @touchmove.stop.prevent="moveHandle">
			...
			<view class="cu-modal" :class="{ show: update.update }">
				<view class="cu-dialog">
					<view class="hade">
						<image
							:src="$flbooth.appstc('/common/update.png')"
							mode="aspectFit"
						></image>
						<view class="title">
					...
					<view class="info">
						...
						<view class="margin-top-xl" v-if="update.download.start">
							...
							<view class="wanl-gray-dark text-sm text-center">
								<text>
									下载中，请稍等（{{
										$flbooth.conver(update.download.totalBytesWritten)
									}}/{{
										$flbooth.conver(update.download.totalBytesExpectedToWrite)
									}}）
								</text>
							</view>
						</view>
...
import flboothPage from '@/components/wanl-shop/page';
import flboothFind from '@/components/wanl-shop/find';
import flboothCategory from '@/components/wanl-shop/category';
		...
		uni.getNetworkType({
			success: res => {
				if (res.networkType == '2g' || res.networkType == '3g' || res.networkType == '4g') {
					this.$flbooth.msg('当前使用非WIFI环境，请注意流量使用');
				} else if (res.networkType == 'none') {
					this.$flbooth.msg('没有网络');
				}
			}
		});
	},
				...
				success: res => {
					...
					switch (QRcode.QRtype) {
						case 'goods':
							...
							break;
						case 'user':
							this.$flbooth.to(`/pages/user/info?id=${QRcode.id}`);
							break;
						case 'category':
							this.$flbooth.on('/pages/category');
							break;
						case 'page':
							this.$flbooth.to(`/pages/page/index?id=${QRcode.id}`);
							break;
						case 'shop':
						...
						case 'live':
							...
							this.$flbooth.auth(`/pages/shop/live/live`);
							// #endif
							// #ifndef APP-PLUS || MP-WEIXIN
							this.$flbooth.msg('目前只开放App和微信小程序直播');
							// #endif
							break;
						case 'chat':
