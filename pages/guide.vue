<template>
	<view class="wanlguide-container">
		<swiper :style="{ height: $flbooth.wanlsys().windowHeight + 'px' }" circular="true" :duration="duration"
			@change="swiperChange">
			<swiper-item class="item text-center" :style="{ backgroundColor: item.url.split('/')[2] }"
				...
				<image :src="$flbooth.oss(item.media, 280, 0, 1, 'transparent', 'png')" mode="aspectFit"></image>
			</swiper-item>
		</swiper>
	...
	export default {
		...
		methods: {
			...
			launchFlag() {
				...
				this.$flbooth.to('/pages/home', 'none', 0);
			}
		}
	};
</script>
