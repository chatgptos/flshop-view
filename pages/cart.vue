		...
		<view class="cu-custom" :style="{ height: $flbooth.wanlsys().height + 'px' }">
			<view
				...
				:style="{
					...
					backgroundImage: 'url(' + $flbooth.oss( common.appStyle.cart_nav_image, 0, 50, 1, 'transparent', 'png' ) + ')',
			...
			>
				...
				<view class="wanl-cart-goods" v-for="(goods, keys) in item.products" :key="keys">
					...
					<view class="picture" @tap="onGoods(goods.goods_id)">
						<image
							:src="$flbooth.oss(goods.image, 100, 100, 1)"
							mode="aspectFill"
						></image>
					</view>
					<view class="content">
...
export default {
	...
	methods: {
		...
		async loadlikeData() {
			this.$api.get({
				url: '/flbooth/product/likes?pages=cart',
				data: {
