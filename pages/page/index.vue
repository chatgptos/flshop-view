<template>
	<view class="wanl-page" :style="{
			...
			backgroundImage : 'url(' + $flbooth.oss(pageData.style.pageBackgroundImage, 0, 50, 1, 'transparent', 'png') + ')'
		}">
		<!-- 导航条 -->
		<view class="cu-custom" :style="{height: $flbooth.wanlsys().height + 'px' }">
			<view class="cu-bar fixed" :style="{
				...
				backgroundImage : 'url(' + $flbooth.oss(pageData.style.navigationBackgroundImage, 414, 0, 1, 'transparent', 'png') + ')'
			}">
...
export default {
	...
	methods: {
		...
		async loadData(option) {
			this.$api.get({
				url: '/flbooth/page/index',
				data: option,
				success: res => {
					this.itemData = res.item;
					this.pageData = res.page;
					//修改页面标题、导航条前景色frontColor和背景backgroundColor
					this.$flbooth.title(res.page.params.navigationBarTitleText,{
