<template>
	<view class="wanl-list">
		...
		<uni-drawer :visible="showRight" mode="right" @close="closeDrawer">
			<view class="drawer">
				<scroll-view scroll-y="true" class="scroll" :style="{ height: $flbooth.wanlsys().windowHeight + 'px' }">
...
export default {
	...
	methods: {
		...
		async loadlikeData() {
			this.$api.get({
				url: `/flbooth/product/likes?pages=${this.goodsType}`,
				success: res => {
		...
		search() {
			this.$flbooth.to(`/pages/page/search?type=${this.goodsType}`,'fade-in',100);
		},
		editListstyle() {
