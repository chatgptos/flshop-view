	...
	<view>
		...
		<view class="wanl-goods-comment">
			...
			<view class="list padding-bj radius-bock" v-for="item in listData" :key="item.id">
				<view class="userinfo">
					<view class="avatar">
						<view class="cu-avatar round margin-right-xs" :style="{ backgroundImage: 'url(' + $flbooth.oss(item.user.avatar, 35, 35, 2, 'avatar') + ')' }"></view>
						<view class="text-sm">
							<view>{{ item.user.nickname }}</view>
							<view class="text-gray">{{ $flbooth.timeToDate(item.created) }}</view>
						</view>
					</view>
	...
	methods: {
		...
		previewImage(image, index) {
			...
			for (var item of image) {
				imgArr = imgArr.concat(this.$flbooth.oss(item, 500, 0));
			}
			uni.previewImage({
				urls: imgArr,
