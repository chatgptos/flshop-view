	...
	<view>
			...
			<scroll-view class="scroll-view padding-bj" scroll-x="true">
		        <view class="list">
		        	<view class="item" v-for="(item, index) in data.goods" :key="item.id" @tap="onGoods(item.goods_id)">
		        		<view class="cu-avatar lg radius-bock" :style="{ backgroundImage: 'url(' + $flbooth.oss(item.image, 'order') + ')' }"></view>
					</view>
		        </view>
		    </scroll-view>
		</view> -->
		...
		<view class="margin-bj bg-white radius-bock" v-if="isLoad">
			<view class="flex align-center margin-bj" @tap="phoneCall(data.kuaidi.tel)">
				<view class="cu-avatar lg radius-bock bg-white" :style="{ backgroundImage: 'url(' + $flbooth.oss(data.kuaidi.logo, 40,40) + ')' }"></view>
...
export default {
	...
	methods: {
		...
		async loadData(data) {
			this.$api.post({
				url: '/flbooth/groups/order/getLogistics',
				loadingTip: '加载中',
				data: data,
				success: res => {
					this.data = res;
