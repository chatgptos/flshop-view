						...
						<view class="swiper-item">
							<view class="user">
								<view class="cu-avatar round margin-right-sm"
									:style="{ backgroundImage: 'url(' + $flbooth.oss(item.user.avatar, 52, 52, 2, 'avatar') + ')' }">
								</view>
			...
			<view class="user" v-for="(item, index) in goodsData.comment_list.data" :key="item.id" @tap="onTag('')">
				<view class="userinfo">
					<view class="avatar">
						<view class="cu-avatar sm round margin-right-xs"
							:style="{ backgroundImage: 'url(' + $flbooth.oss(item.user.avatar, 26, 26, 2, 'avatar') + ')' }">
						</view>
				...
				<view class="grid flex-sub grid-square"
					...
					<view class="bg-img" v-for="(image, index) in item.images" :key="index" v-if="index <= 3"
						:style="{ backgroundImage: 'url(' + $flbooth.oss(image, 88, 88) + ')' }"></view>
				</view>
			</view>
			<!-- 查看更多 -->
		...
		<view class="shop padding-bj solid-bottom bg-white">
			<view class="shopinfo margin-bottom">
				<view class="cu-avatar lg radius-bock"
					:style="{ backgroundImage: 'url(' + $flbooth.oss(goodsData.shop.avatar, 52, 52, 2, 'avatar') + ')' }">
				</view>
				<view class="shopname">
					<view class="text-df text-cut" style="width: 300rpx;">{{ goodsData.shop.shopname }}</view>
					<view class="wanl-gray text-min">{{ $flbooth.toFormat(goodsData.shop.find_user.fans, 'thousand') }} 人关注店铺
					</view>
				</view>
				<view class="bnt">
					<!-- 1.0.9升级 -->
					...
					<view class="wanl-modal">
						...
						<scroll-view class="wanl-coupon scroll-y" scroll-y="true">
							<view v-for="(coupon, index) in goodsData.coupon" :key="index" :class="coupon.type"
								...
								<image :src="$flbooth.appstc('/coupon/img_couponcentre_received_3x.png')"
			...
			<view class="cu-modal bottom-modal" :class="modalName == 'share' ? 'show' : ''" @tap="hideModal">
				<view class="cu-dialog" @tap.stop="">
					<wanl-share :scrollAnimation="scrollAnimation" :shareTitle="goodsData.title"
						:shareText="goodsData.description" :image="$flbooth.oss(goodsData.image,50, 50)"
	...
	export default {
		...
		methods: {
			async loadData(option) {
				//加载商品
				this.$api.get({
					url: '/flbooth/groups/product/goods',
					data: option,
					success: res => {
						//详情正则
						res.content = res.content.replace(/<img [^>]*src=['"]([^'"]+)[^>]*>/gi, (match,
							capture) => {
							var img =
								`<img style="display: block; max-width: 100%;" src="${this.$flbooth.oss(capture, 500, 0)}">`;
							return img;
						});
						// 价格区间
						if (res.sku.length == 0) {
			...
			async loadStock(sku_id){
				this.$api.get({
					url: '/flbooth/groups/product/stock',
					data: {
						sku_id: sku_id
					},
					success: res => {
			...
			groupPrice(){
				...
				if(this.selectshop.price){
					if(this.ladderIndex !== null && this.goodsData.is_ladder === 1){
						price = this.$flbooth.bcmul(this.selectshop.price, this.goodsData.groups.ladder[this.ladderIndex].discount) / 100;
					}else{
			...
			wanlShare(e) {
				if (e) {
					this.$flbooth.auth(`/pages/user/complaint/complaint?id=${this.goodsData.id}&type=3`);
				} else {
					this.modalName = null;
				}
			},
			...
			follow() {
				...
				this.$api.post({
					url: '/flbooth/groups/product/follow',
					data: {
			...
			// 评论页
			onTag(tag) {
				this.$flbooth.to('/pages/product/comment?id=' + this.goodsData.id + '&praise=' + this.goodsData.praise +
					'&tag=' + tag);
			},
			// 打开购物车
			toCart() {
				...
				if (this.isChoice) {
					...
					if (this.$store.state.user.isLogin) {
						if(this.goodsData.purchase_limit === 0 || this.isChoice === 'alone'){
							...
							// 查询是否超过限制
							this.$api.get({
								url: '/flbooth/groups/order/getOrderPurchaseLimit',
			...
			productSearch(text) {
				this.$flbooth.to(`/pages/page/search?type=groups&keywords=${text}`, 'fade-in', 100);
			}
		}
	};
</script>
