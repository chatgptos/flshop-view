<template>
	<view class="wanlgroups-user-container">
		...
		<view
			...
			:style="{
				backgroundColor: userData.color,
				backgroundImage: `url(${$flbooth.oss(
					userData.image,
					0,
					50,
					1,
			...
			>
				<view class="userinfo">
					<view
						...
						:style="{
							backgroundImage: `url(${$flbooth.oss(userData.user.avatar, 88, 88)})`
						}"
					/>
					<view>
						...
						<view class="text-xs text-white-agba">
							...
							<text
								...
								@click.stop="$flbooth.copy(userData.user_no)"
							></text>
						</view>
					</view>
				</view>
				...
				<view class="findinfo margin-top">
					<view class="quantity">
						<view
							class="project"
							@click.stop="
								$flbooth.to(`/pages/apps/find/user/follow?id=${userData.user_no}`)
							"
						>
							<view class="text-lg">
								{{
						...
						<view
							class="project"
							@click.stop="
								$flbooth.to(`/pages/apps/find/user/fans?id=${userData.user_no}`)
							"
						>
							<view class="text-lg">
								{{
				...
				>
					<view
						class="cu-avatar margin-right-bj round"
						:style="{
							backgroundImage: `url(${$flbooth.oss(userData.shop.avatar, 88, 88)})`
						}"
					/>
					<view class="subject">
						<view class="info">
					...
					<block v-if="isMyfind">
						<view v-if="data.dataList.length === 0 && data.loaded" class="empty">
							<view class="text-center wanl-gray">
								<image
									:src="$flbooth.appstc('/default/collect_default3x.png')"
								></image>
								...
								<button
									v-else
									class="cu-btn line-black"
									@click="$flbooth.to('/pages/apps/find/list')"
								>
									去看看
								</button>
							</view>
					...
					<view class="list" v-if="data.dataList.length > 0">
						<view class="item" v-for="(item, index) in data.dataList" :key="index">
							<view
								...
								:style="{
									backgroundImage:
										'url(' + $flbooth.oss(item.images[0], 200, 0) + ')'
								}"
								@tap="onFind(item)"
							>
								<view
							...
							<view
								v-else
								class="examine radius-bock"
								@tap="$flbooth.msg(`内容${stateText[item.state].text}, 无法查看`)"
							>
								<view
									class="cover"
									:style="{
							...
							<view class="flex wanl-gray text-min padding-lr-xs justify-between">
								<view>
									<text>
										{{ $flbooth.timeFormat(item.created, 'mm-dd') }} 发布
									</text>
									<text
		...
		<view class="WANL-MODAL wanlgroups-user-container__modal">
			...
			>
				<view class="cu-dialog" @tap.stop="">
					<image
						:src="$flbooth.appstc('/common/find_fans3x.png')"
						mode="widthFix"
					></image>
					<view class="margin-tb-lg">
						<text class="wanl-gray-dark">
							{{
								`“${userData.user.nickname}”共收获得 ${$flbooth.toFormat(
									userData.praised < 0 ? 0 : userData.praised,
									'thousand'
								)} 个赞`
							}}
...
export default {
	...
	methods: {
		async loadData(data) {
			this.$api.post({
				url: '/flbooth/find/user/userInfo',
				data: data,
				success: res => {
				...
				uni.showActionSheet({
					...
					success: action => {
						if (action.tapIndex === 0) {
							this.$flbooth.to('/pages/user/theme');
		...
		editTheme(data) {
			this.$api.post({
				url: '/flbooth/find/user/editTheme',
				loadingTip: '更改主题中...',
				data: data,
				success: res => {
			...
			uni.showActionSheet({
				...
				success: res => {
					if (res.tapIndex === 2) {
						...
						this.$flbooth.to(url[res.tapIndex]);
						// #endif
		...
		handleUserEdit() {
			this.$flbooth.auth('/pages/user/setting/user');
		},
		// 格式化html
		formatHtml(content) {
