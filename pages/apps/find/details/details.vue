	...
	<view>
		...
		<view class="cu-custom" :style="{ height: $flbooth.wanlsys().height + 'px' }">
			...
			>
				...
				<view class="content" :style="{ top: $flbooth.wanlsys().top + 'px' }">
					...
					<view v-else class="userinfo">
						<block v-if="data.shop">
							<view
								...
								:style="{
									backgroundImage: `url(${$flbooth.oss( data.shop.avatar, 35, 35, 2, 'avatar' )})`
								}"
							></view>
						...
						<block v-else>
							<view
								class="cu-avatar round"
								:style="{
									backgroundImage: `url(${$flbooth.oss( data.user.avatar, 35, 35, 2, 'avatar' )})`
								}"
							></view>
							<view class="text-cut margin-lr-xs text-sm">
								{{ data.user.nickname }}
		...
		<view class="wanl-find bg-white" id="main">
			...
			<view class="content padding-bj" v-if="data.type == 'new'">
				<view class="flex align-center">
					<view
						...
						:style="{
							backgroundImage: `url(${$flbooth.appstc('/common/calendar.png')})`
						}"
					>
						{{ data.created_date }}
					</view>
			...
			<view class="content" v-if="data.type == 'want' || data.type == 'show'">
				<view class="swiper-box">
					<swiper circular="true" autoplay="true" @change="swiperChange($event)">
						<swiper-item v-for="(swiper, index) in data.images" :key="index">
							<image :src="$flbooth.oss(swiper, 400, 0)" mode="aspectFill"></image>
						</swiper-item>
					</swiper>
				...
				>
					...
					>
						<view class="img">
							<image :src="$flbooth.oss(goods.image, 50, 50)"></image>
						</view>
			...
			<view class="flex justify-between padding-bj align-center">
				<view @tap="handleComments()" v-if="common.appConfig.comment_switch == 'Y'">
					<view
						...
						:style="{
							backgroundImage: `url(${$flbooth.oss( user.avatar, 35, 35, 2, 'avatar' )})`
						}"
					></view>
		...
		>
				...
				<view v-if="data.shop" class="avatar">
					...
					<view>
						...
						<view class="wanl-gray text-sm">
							{{ $flbooth.timeToDate(item.created) }}
						</view>
					</view>
				</view>
			...
			<view class="content" v-if="item.type == 'video'">
				<view class="video-item radius-bock" @tap="handleVideo(item.id)">
					<image :src="$flbooth.oss(item.video.cover_url, 400, 0)" mode="aspectFill"></image>
...
export default {
	...
	methods: {
		async loadData() {
			this.$api.get({
				url: '/flbooth/find/find/getDetails',
				data: {
					id: this.find_id
				},
				success: res => {
		...
		async handleComments(index = null) {
			let data = index === null ? this.data : this.listData[index];
			this.$api.get({
				url: '/flbooth/find/comments/getList',
				data: {
					id: data.id
				},
				success: res => {
