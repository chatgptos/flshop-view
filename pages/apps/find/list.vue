<template>
	<view class="flbooth-find-container">
		<view class="flbooth-find-container___head" :style="{height: `${headHeight}px`}">
			<view class="fixed bg-bgcolor" :style="{height: `${headHeight}px`, paddingTop: `${headTop}px`}">
				<view class="head">
					...
					<view class="search round padding-lr-bj solid" @tap="$flbooth.to('/pages/page/search?type=find')">
		...
		<view class="flbooth-find-container__main">
					...
					<view class="find">
						<view class="item" v-for="(item, index) in navList[currentId].dataList" :key="index">
							<view class="cover radius-bock"
								:style="{ backgroundImage: 'url(' + $flbooth.oss(item.images[0], 200, 0) + ')' }"
								@tap="onFind(item)">
							</view>
						...
						<view class="item padding-bj margin-bj bg-white radius-bock" v-for="(item, index) in navList[currentId].dataList" :key="index" @click="handleUser(item.user_no)">
							<block v-if="item.shop">
								<view class="cu-avatar margin-right-bj round"
									:style="{backgroundImage: `url(${$flbooth.oss(item.shop.avatar, 88, 88)})`}"
								>
								</view>
								<view class="subject">
									<view class="info">
							...
							<block v-else>
								<view class="cu-avatar margin-right-bj round"
									:style="{backgroundImage: `url(${$flbooth.oss(item.user.avatar, 88, 88)})`}">
								</view>
								<view class="subject">
									<view class="info">
