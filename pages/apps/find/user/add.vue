				...
				<view class="grid col-4 grid-square flex-sub padding-lr-bj">
					...
					>
						<view class="item" @tap="handlePreview(index)">
							<image :src="$flbooth.oss(item, 100, 100)" mode="widthFix"></image>
						</view>
						...
						<view v-if="video.progress > 0 && video.progress !== 100" class="loading flex align-center justify-center">
							<view class="text-xxl text-gray text-center">
								...
								<view class="margin-top-xs text-min">
									{{$flbooth.conver(video.totalBytesSent, false)}}/{{$flbooth.conver(video.totalBytesExpectedToSend)}}
								</view>
							</view>
						</view>
					</view>
			...
			<view class="solid-top">
				...
				<view class="grid col-4 grid-square flex-sub padding-lr-bj">
					...
					>
						<view class="item">
							<image
								:src="$flbooth.oss(item.image, 100, 100)"
								mode="widthFix"
							></image>
						</view>
					</view>
...
export default {
	...
	methods: {
		...
		handleSubmit() {
			...
			if(params.type === 'video'){
				...
				if(video.tempFilePath === null ||  params.video_id === null ){
					this.$flbooth.msg('请上传视频');
					return;
				}else{
					if(video.progress === 0){
						this.$flbooth.msg('异常请重开此页面');
						return;
					}
					if(video.progress < 100 || video.totalBytesSent !== video.totalBytesExpectedToSend){
						this.$flbooth.msg('视频还没有上传完成哦~');
						return;
					}
				}
				if(!params.content){
					this.$flbooth.msg('请编辑视频标题~');
					return;
				}
			}else if(params.type === 'want'){
				if(params.images.length === 0){
					this.$flbooth.msg('至少添加一张图片');
					return;
				}
				if(!params.content){
					this.$flbooth.msg('请编辑种草详情~');
					return;
				}
			}
			if(params.goods_ids.length === 0){
				this.$flbooth.msg('至少选择一个商品');
				this.handleGoods();
				return;
			}
			this.$api.post({
				url: '/flbooth/find/find/addData',
				data: params,
				loadingTip: '发布中...',
				success: res => {
					this.$flbooth.msg('发布成功！');
					this.$flbooth.to('/pages/page/success?type=find');
				}
			});
		},
		// 选择视频
		handleVideo(e) {
			uni.chooseVideo({
				success: ch => {
					let name = ch.name ? ch.name : `${this.$flbooth.guid(12)}.mp4`;
					this.$api.get({
						url: '/flbooth/video/getUploadProof',
						data: {
							name: name,
						},
						success: sts => {
