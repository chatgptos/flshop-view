<template>
	<view>
		...
		<view class="cu-custom" :style="{height: $flbooth.wanlsys().height + 'px' }">
			<view class="cu-bar bg-bgcolor fixed" :style="{ height: $flbooth.wanlsys().height + 'px', paddingTop: $flbooth.wanlsys().top + 'px'}">
				...
				<view class="action" @tap="$flbooth.to('/pages/user/service')">
		...
		<form @submit="formSubmit">
			...
			<view class="cu-form-group margin-top-bj" :class="shopdata.state == 2 ? '':'margin-bottom-bj'">
				...
				<view class="cu-avatar radius" v-if="shopdata.image" @tap="previewImage(0)">
					<image :src="$flbooth.oss(shopdata.image, 32, 0, 1)" mode="widthFix"></image>
				</view>
			</view>
			<view class="cu-form-group margin-bottom-bj" v-if="shopdata.state == 2">
				...
				<view class="cu-avatar radius" v-if="shopdata.trademark" @tap="previewImage(1)">
					<image :src="$flbooth.oss(shopdata.trademark, 32, 0, 1)" mode="widthFix"></image>
				</view>
			</view>
		...
		methods: {
			...
			formSubmit(e) {
				...
				if (this.shopdata.verify != 3) {
					...
					this.$flbooth.msg('已成功入驻，不可修改再修改申请信息');
				}
			},
			async wanlChecker(e){
				//定义表单规则
				...
				if(checkRes){
					...
					this.$api.post({
						url: '/flbooth/shop/apply',
						data: shopdata,
						success: res => {
							this.shopdata = res;
							if (shopdata.verify == 0) {
								shopdata.verify = 2;
								this.$flbooth.msg('提交成功，平台将于7个工作日内与您联系');
							}else{
								shopdata.verify = 5;
								this.$flbooth.msg('修改成功！');
							}
						}
					});
				}else{
			...
			previewImage(key) {
				...
				if (shopdata.state == 2) {
					urls = [this.$flbooth.oss(shopdata.image, 320, 0, 1), this.$flbooth.oss(shopdata.trademark, 320, 0, 1)];
				}else{
					urls = [this.$flbooth.oss(shopdata.image, 320, 0, 1)];
				}
				uni.previewImage({
					urls: urls,
					current: urls[key],
