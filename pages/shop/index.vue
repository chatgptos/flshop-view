<template>
    <view class="flbooth-shop-container" :style="{
		backgroundImage: `url()`
	}">
		<!-- 顶部导航 -->
		<view :style="{ height: `${sys.headHeight}px`}">
			<wanl-shop-head
			class="flbooth-shop-container__head"
		...
		<!-- 主体内容 -->
		<view class="flbooth-shop-container__main">
			<swiper class="page"
		...
		<!-- 底部导航 -->
		<view class="flbooth-shop-container__tabbar" :style="{ height: `${sys.tabbarHeight}px`, paddingBottom:`${sys.tabbarBottom}px` }">
			<view class="cu-bar tabbar">
				<view class="action" @tap="handleTabbar('home')">
					<view v-if="handleActive('home')">
						<view class="cu-avatar round" :style="{backgroundImage: `url(${$flbooth.oss(shopData.avatar, 32, 32)})`}" />
					</view>
					<view v-else>
				...
				<view class="action">
					<view v-if="user.id === shopData.user_id" @tap="$flbooth.to('/pages/apps/find/user')">
...
<script>
	...
	import flboothHead from '@/components/wanl-shop/shop/head';
	import flboothProduct from '@/components/wanl-shop/shop/product';
	import flboothFind from '@/components/wanl-shop/shop/find';
	export default {
	    data() {
	        return {
				tabbarId: 'home',
