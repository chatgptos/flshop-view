 ...
 **/
 ...
 import fun from '../../common/flbooth_function';
 ...
 export default {
 	...
 	actions: {
		...
		async del({state, dispatch, rootState}, index){
			...
			// 操作云
			api.post({url: '/flbooth/chat/del',data: {id: list[index].user_id}});
			// 删除状态管理器
		...
		async update({state, dispatch}, {type, data, id, shop}){
			if (type == 'del') {
				...
				// 操作云
				api.post({url: '/flbooth/chat/clear',data: {id: id}});
		...
		async storage({state, rootState}, {type, number}) {
			...
			uni.setStorageSync("flbooth:statis", rootState.statistics);
		},
		// 关闭即时通讯
		async close(){
			uni.onSocketOpen(()=> {
