...
import flbooth_function from "./common/flbooth_function";
import flbooth_config from './common/config/config';
...
Vue.mixin(mpShare);
// 阻止启动生产消息，常用作指令。
Vue.config.productionTip = flbooth_config.debug;
// 挂载
...
// 配置请求
api.setConfig({
	baseUrl: flbooth_config.appurl,
	debug: flbooth_config.debug
})
...
api.interceptor.request = (config => {
	...
	if (!config.header.token || !config.header.flbooth) {
		let wanlLogin = uni.getStorageSync("flbooth:user");
		if (wanlLogin) {
			config.header.token = uni.getStorageSync("flbooth:user").token;
		}
		// #ifdef APP-PLUS
		config.header.uuid = plus.device.uuid;
		config.header['App-Client'] = 'app-flbooth';
		// #endif
		// #ifdef H5
		if(jssdk.isWechat()){
			config.header['App-Client'] = 'wechat-flbooth';
		}else{
			config.header['App-Client'] = 'h5-flbooth';
		}
		// #endif
		// #ifdef MP
...
api.interceptor.response = ((res, config) => {
	...
	flbooth_config.debug?console.log(res):'';
	return res;
})
 ...
 */
const appstc = (url) => {
	return flbooth_config.cdnurl + '/assets/addons/flbooth/img' + url;
}
 ...
 */
const bcadd = (a, b) => {
	return flbooth_function.bcadd(a, b);
}
 ...
 */
const bcsub = (a, b) => {
	return flbooth_function.bcsub(a, b);
}
 ...
 */
const bcmul = (a, b) => {
	return flbooth_function.bcmul(a, b);
}
 ...
 */
const bcdiv = (a, b) => {
    return flbooth_function.bcdiv(a, b);
};
...
const oss = (url, w = 120, h = 120, modenum = 2, type = '', format = 'jpg', resize = 'resize', multiple = 3) => {
	...
	if (url) {
		if ((/^data:image\//.test(url))) {
			...
			image = type == 'video' ? (flbooth_config.cdnurl + url) : (flbooth_config.cdnurl + url + rule.join("/"));
		}
	}else{
		if (type == 'transparent') {
...
const to = (url, animationType = 'pop-in', animationDuration = 300) => {
	uni.navigateTo({
		...
		fail: function (e) {
			flbooth_config.debug?console.log(e):'';
		}
	})
}
...
const send = (data) => {
	...
	api.post({
		...
		data: data,
		success: res => {
			flbooth_function.setChat(data, 'send');
		}
	});
}
